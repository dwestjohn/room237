...
function CreateInventoryPage() {
  ...
  const fetchTypes = async () => {
    const res = await api.get('/api/inventory/types');
    setTypes(res.data);
  };
  ...
  const fetchVessels = async () => {
    const res = await api.get('/api/recipes/vessels');
    setVessels(res.data);
  };

  const fetchRecipeTypes = async () => {
    const res = await api.get('/api/recipes/types');
    setRecipeTypes(res.data);
  };

  const handleCreateType = async () => {
    try {
      await api.post('/api/inventory/types', {
        type_name: typeName,
    ...
    <div className="container">
      ...
      <div className="card">
        ...
        <button className="order-button" onClick={async () => {
          try {
            await api.post('/api/recipes/vessels', { method, glassware });
            setMethod('');
            setGlassware('');
            fetchVessels();
      ...
      <div className="card">
        ...
        <button className="order-button" onClick={async () => {
          try {
            ...
            await api.post('/api/recipes/ingredients', {
      ...
      <div className="card">
        ...
        <button className="order-button" onClick={async () => {
          ...
          try {
            await api.delete(`/api/inventory/types/${typeToDelete}`);
            alert('Type deleted!');
            setTypeToDelete('');
            fetchTypes();
          } catch (err) {
